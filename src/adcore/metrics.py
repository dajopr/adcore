...
Every pixel score is a functional of two counting measures over a
shared threshold grid: how many defect-free pixels sit at or above t, and how many defect
pixels do. Computing those once per image turns every subset evaluated (a category, a defect type)
from a full pass over ~39M pixels into a sum over an `(n_images, n_bins)` array.
...
rows a subset selects — which is the whole reason a subset costs a sum rather than a pass.
...
class ScoreBins:
    ...
    The grid is built from every pixel of an eval set and never rebuilt per subset. Two
...
class PixelHistograms:
    ...
    Rows are positionally aligned with the images passed in, so a subset is a set of row
...
def _as_image_stack(array: np.ndarray) -> np.ndarray:
    """Drop a singleton channel axis on masks or maps, accepting both without one."""
...
def build_pixel_histograms(
    ...
    *,
    ...
    """One pass over every image, building the statistics every subset re-aggregates.

    `masks` and `anomaly_maps` may each be (N, H, W) or (N, 1, H, W).
    """
...
def pixel_metrics(
    ...
    *,
    ...
    Degenerate selections return NaN rather than raising: a subset whose rows hold no
    defect pixels has no AUROC, and one with no regions has no AUPRO. sklearn raises on the
    first.
    """
    if rows is None:
...
def image_auroc(labels: np.ndarray, scores: np.ndarray) -> float:
    """Image-level AUROC; NaN when `labels` holds only one class instead of raising."""
